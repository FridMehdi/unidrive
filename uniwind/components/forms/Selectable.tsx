  ...
  return (
    <View className={`mb-2 ${containerClassName}`} >
      <Pressable
        ...
        style={style}
        className={twMerge(
          'border rounded-2xl active:opacity-70 bg-secondary',
          containerPadding,
          selected ? 'border-highlight' : 'border-transparent',
      ...
      >
        <View className="flex-row items-center">
          {icon && (
            <View className={`mr-4 h-16 w-16 rounded-xl items-center justify-center ${selected ? 'bg-highlight' : 'bg-background'}`}>
              <Icon 
                ...
                size={24} 
                strokeWidth={1.5}
                color={iconColor || (selected ? colors.text : colors.icon)}
              />
            </View>
          )}
          {customIcon && (
          ...
          <View className="flex-1">
            ...
            {description && (
              <ThemedText className="text-sm text-text mt-0">
                {description}
              </ThemedText>
            )}
          </View>
